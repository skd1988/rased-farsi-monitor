...
interface TopTargetedPersonsChartProps {
  data: PersonData[];
  onPersonClick?: (person: string) => void;
}

const TopTargetedPersonsChart: React.FC<TopTargetedPersonsChartProps> = ({ 
  data, 
  onPersonClick 
}) => {
  ...
  return (
    <Card className="shadow-lg hover:shadow-xl transition-shadow">
      <CardHeader>
        ...
        <CardDescription>
          10 شخصیت برتر مورد هدف در حملات جنگ روانی
        </CardDescription>
      </CardHeader>
      <CardContent>
        {data.length === 0 ? (
          ...
          <div className="space-y-4">
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <div className={`text-xs font-medium mt-1 ${
                      person.severity === 'Critical' ? 'text-red-600' :
                      person.severity === 'High' ? 'text-orange-600' :
                      person.severity === 'Medium' ? 'text-yellow-600' :
                      'text-green-600'
                    }`}>
                      {person.severity === 'Critical' ? 'بحرانی' :
                       person.severity === 'High' ? 'بالا' :
                       person.severity === 'Medium' ? 'متوسط' : 'پایین'}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Tooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload[0]) {
                      const data = payload[0].payload as PersonData;
                      return (
                ...
                <Bar 
                  dataKey="count" 
                  radius={[0, 4, 4, 0]}
                  onClick={(data) => onPersonClick?.(data.name_english || data.name_persian)}
            ...
            <div className="flex items-center justify-between pt-4 border-t text-sm">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span>مجموع حملات: {data.reduce((sum, p) => sum + p.count, 0)}</span>
              </div>
              <div className="text-muted-foreground">
                {data.length} شخصیت مورد هدف
              </div>
            </div>
          </div>
        )}
...
export default TopTargetedPersonsChart;
