...
const Settings = () => {
  ...
  const deleteAllPosts = async () => {
    ...
    const { count: channelsCount } = await supabase.from("social_media_channels").select("*", { count: "exact", head: true });
    const { count: sourcesCount } = await supabase.from("source_profiles").select("*", { count: "exact", head: true });
    ...
    try {
      ...
      while (hasMoreChannels) {
        const { data: batch } = await supabase.from("social_media_channels").select("id").limit(100);
        if (!batch || batch.length === 0) {
      ...
      while (hasMoreSources) {
        const { data: batch } = await supabase.from("source_profiles").select("id").limit(100);
        if (!batch || batch.length === 0) {
